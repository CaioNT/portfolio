document.addEventListener("DOMContentLoaded", function () {
  ...
  const logoHref = document.querySelectorAll(".logo-href");
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    emailjs
      .send("service_b7uypot", "template_qlonzw9", templateParams)
      .then((response) => {
      ...
      .catch((err) => {
        ...
        alert(
          `Erro ao enviar: ${err.text || err.message || JSON.stringify(err)}`
        );
        submitBtn.innerHTML = originalText;
        submitBtn.disabled = false;
      });
